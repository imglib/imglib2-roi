...
public class Demo
{
	public static void testAnd()
	{
		...
		final RealRandomAccessibleRealInterval< BoolType > rrari = Masks.toRRARI( and1 );
		final Bdv bdv = BdvFunctions.show( rrari, Intervals.smallestContainingInterval( rrari ), "Box AND Sphere", Bdv.options() );
		/*
		 * NEW: Side note: Use
		 *   Intervals.smallestContainingInterval( rrari )
		 * instead of
		 *   new FinalInterval(
		 ...
		 * Works for all dimensionalities and is more correct than rounding.
		 */

		final BdvSource box = BdvFunctions.show( Masks.toRRARI( b ), Intervals.smallestContainingInterval( b ), "Box", Bdv.options().addTo( bdv ) );
