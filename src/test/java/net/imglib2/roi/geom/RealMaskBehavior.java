...
public class RealMaskBehavior
{
	...
	public static void test3D()
	{
		...
		final Mask< RealLocalizable > affineRect = Masks.affineTransform(
				rect,
				affine );

		final SuperEllipsoid ellipse = new ClosedSuperEllipsoid(
				new double[] { 50, 100, 150 },
				new double[] { 50, 100, 150 },
				4 );

		final Mask< RealLocalizable > xorCombined = Masks.realXor( affineRect, ellipse );

		final RealRandomAccessible< BoolType > rra =
				new MaskAsRealRandomAccessible<>( xorCombined, new BoolType() );
	...
	{
		final Polygon2D polygon = new ClosedPolygon2D(
				x,
				y );

		final RealRandomAccessible< BoolType > rra =
				new MaskAsRealRandomAccessible<>( polygon, new BoolType() );
